...
class RequestIndex extends Component{
    ...
    static async getInitialProps(props){
        ...
        const completeRequests = requests.filter(request=>request.complete==true)
        const pendingRequests = requests.filter(request=>request.complete==false)

        
        return {address, completeRequests,pendingRequests, requestCount, contributorCount, managerAddress,userAddress}
                    ...
                    <Table.HeaderCell colSpan='7'>
                        <Link route={'/campaigns/'+this.props.address+'/requests/new'}>
                                ...
                                <Button
                                    ...
                                    color='green'
                                    size='small'
                                    loading={this.state.loading} onClick={(event)=>{this.setState({loading:true})}}
                                >
        ...
        return(
            <Layout>
                ...
                <Table celled striped>
                    <Table.Header>
                    <Table.Row>
                        <Table.HeaderCell colSpan='7'>Finalized requests</Table.HeaderCell>
                    </Table.Row>
                    <Table.Row>
                        <Table.HeaderCell>#</Table.HeaderCell>
                        <Table.HeaderCell>Description</Table.HeaderCell>
