...
class RequestRow extends Component {
  ...
  onApprove = async () => {
    ...
    try {
      ...
      await Campaign(this.props.address)
        .methods.approveRequest(this.props.id)
  ...
  onFinalize = async () => {
    ...
    try {
      const accounts = await web3.eth.getAccounts();
      await Campaign(this.props.address)
        .methods.finalizeRequest(this.props.id)
        .send({ from: accounts[0] });
      Router.replaceRoute("/campaigns/" + this.props.address + "/requests");
    } catch (error) {
  ...
  render() {
    const readyToFinalize =
      this.props.request.approvalCount > this.props.contributorCount / 2;
    return (
      <Table.Row
      ...
      >
        ...
        <Table.Cell>
          {this.props.request.approvalCount}/{this.props.contributorCount}
        </Table.Cell>
        <Table.Cell>
          {this.props.request.complete || readyToFinalize ? (
            <p>Approved</p>
          ) : (
            <Button
