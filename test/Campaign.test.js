...
describe('Campaigns',()=>{
    ...
    it('requires a min contribution',async ()=>{
        try{
            await campaign.methods.contribute().send({value:'1', from:accounts[0]});
            assert(false)
        }
        catch (err){
            assert(err)
    ...
    it('allows manager to create a request',async ()=>{
        await campaign.methods.createRequest(description='Hello',value='100',recepient=accounts[1]).send({from:accounts[0],gas:'1000000'});
    ...
    it('does the job entirely', async ()=>{
        ...
        await campaign.methods.createRequest(description='Hello',value=web3.utils.toWei('5','ether'),recepient=accounts[1]).send({from:accounts[0],gas:'1000000'});
